...
// FinalFantasy6.romからAkaoサウンドドライバ等取得
...
0x4800 - 0x494C　常駐波形BRR（移動ok）
0x494D - 0xXXXX　音源BRR（移動ok）
0xCD00 - 0xF4FF　エコーバッファ（移動ok）
0xF500 - 0xFFFF　ワークメモリ
*/

	// サウンドドライバ
	// 0x05070E 2バイトはサイズ(0x17B7)
...
public:
	...
	FF6_SPC()
	{
		...
		// 0x4800からBRR, 最初の0x14Cまでは常駐波形BRR
		brr_offset = 0x4800;
...
//{FILE *fp=fopen("sample_debug.txt","w");fprintf(fp,str.c_str());fclose(fp);}
	...
	for(p=0; str[p]!='\0'; p++){
			...
			if(str.substr(p, 5)=="#tone"){
				...
				if(brr_fname.substr(0,8)=="FF6inst:"){
					...
					if(f_stayinst && (inst_id<0 || inst_id>7)){
						printf("Error line %d : FF6inst 常駐波形指定は s0～s7 としてください.\n", line);
						return -1;
					}
				...
				if(brr_fname.substr(0,8)=="FF6inst:"){
					if(param_num==0){
						// 無指定
						tone_map[tone_id].octave = 0xFF; // D6 D9 DB を出力しない
						...
						printf("Error line %d : FF6波形指定の場合のパラメータ数は0個か3個です.\n", line);
						return -1;
					}
				}
				else{ // brrファイル指定の場合
		...
		if(str[p]=='@' && str[p+1]=='@'){
			...
			if(id<0 || id>505){
				printf("Error line %d : @@効果音番号は 0～505 までです.\n", line);
				return -1;
			}
		...
		if(str[p]=='@'){ // @3 @12 @B @0C @piano
			...
			if(tone_map[tone_id].brr_fname.substr(0,8)=="FF6inst:" && tone_map[tone_id].octave==0xFF){
				sprintf(buf_octave, "");
				sprintf(buf_transpose, "");
				sprintf(buf_detune, "");
			}
			else{
				sprintf(buf_octave, "D6 %02X", tone_map[tone_id].octave);
				sprintf(buf_transpose, "D9 %02X", tone_map[tone_id].transpose);
				sprintf(buf_detune, "DB %02X", tone_map[tone_id].detune);
			}
			sprintf(buf, "DC %02X %s %s %s ",
				(tone_map[tone_id].brr_fname.substr(0,9)=="FF6inst:s")
	...
	// 未使用toneの警告
	map<string, FF6_TONE>::iterator mit;
...
//	memcpy(ram+0x4800, asd.sbrr, asd.sbrr_size);
	...
	//	if(brr_fname[0]=='\0') continue;
		...
		if(brr_put_map.find(brr_fname)==brr_put_map.end()){ // 置いてないbrrなら
			...
			int brr_size;
			uint8 *brr_data;
			if(brr_fname.substr(0, 8)=="FF6inst:"){ // FF5の波形
...
//printf("%d %s start 0x%X end 0x%X\n", 0x20+i, brr_fname.c_str(), start_adrs, start_adrs+brr_size-2-1);
			if(start_adrs+brr_size-2-1 >= 0x0F500){
				...
				printf("Error : %s BRRエリアが0xF500を超えました.\n", brr_fname.c_str());
				delete[] brr_data;
				delete[] ram;
				return -1;
			}
			...
			if(start_adrs >= 0x0F500){
				...
				printf("Error : %s BRRスタートアドレスが0xF500を超えました.\n", brr_fname.c_str());
				delete[] ram;
				return -1;
			}
			if(loop_adrs >= 0x0F500){
				printf("BRR loop address 0x%X\n", loop_adrs);
				printf("Error : %s BRRループアドレスが0xF500を超えました.\n", brr_fname.c_str());
				delete[] ram;
				return -1;
			}
...
{
	printf("[ spcmake_byFF6 ver.20200919 ]\n\n");
